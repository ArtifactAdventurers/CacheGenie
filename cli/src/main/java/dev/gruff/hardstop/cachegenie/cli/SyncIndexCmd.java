...
public class SyncIndexCmd implements Runnable {
    ...
    @CommandLine.Option(names = {"--merge-batch"}, paramLabel = "<n>",
            description = "On a full pull, merge staged records into the meta tables every N records "
                    + "(default 5000000) instead of in one merge at the end, bounding the size of each "
                    + "staging table and merge transaction. Result is identical (merges are idempotent, "
    ...
    public void run() {
        ...
        new IndexerSyncAction(parent.genie()).sync(full, limit, mergeBatch);
        System.exit(0);
    }
}
